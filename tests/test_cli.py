"""Tests for OptiProxAI CLI — config error handling and init command."""
...
def empty_dir(tmp_path, monkeypatch):
    """Run in a temp dir with no config files and unset OPTIPROXAI_CONFIG."""
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("OPTIPROXAI_CONFIG", raising=False)
    # Also override XDG so it doesn't find real user config
    monkeypatch.setenv("OPTIPROXAI_CONFIG_DIR", str(tmp_path / "xdg_config"))
    return tmp_path
...
class TestConfigErrors:
    ...
    def test_route_no_config_shows_friendly_error(self, runner, empty_dir):
        ...
        assert "No OptiProxAI configuration file found" in result.output
        assert "optiproxai init" in result.output
...
"""
    ...
    def test_keys_add_rejects_empty_label(self, runner, empty_dir) -> None:
        ...
        monkeypatch.setenv("OPTIPROXAI_DATA_DIR", str(empty_dir / "data"))
        try:
    ...
    def test_keys_remove_redacts_secret_shaped_identifier(
        ...
        monkeypatch.setenv("OPTIPROXAI_DATA_DIR", str(empty_dir / "data"))
        try:
            result = runner.invoke(main, ["keys", "remove", "optiproxai-raw-secret"])

            assert result.exit_code != 0
            assert "optiproxai-raw-secret" not in result.output
