...
from optiproxai.api_keys import generate_key, has_keys
from optiproxai.proxy import (
    _proxy_upstream,
...
from optiproxai.router import FallbackEntry, RoutingDecision


@pytest.fixture(autouse=True)
def isolate_data_dir(tmp_path, monkeypatch):
    monkeypatch.setenv("OPTIPROXAI_DATA_DIR", str(tmp_path))
...
class TestProxyFallbackBehavior:
    ...
    async def test_fallbacks_skip_primary_duplicate_and_dedupe(self):
        ...
        with patch("optiproxai.proxy._proxy_upstream", side_effect=fake_proxy_upstream):
...
class TestSuccessfulFailureHandling:
    ...
    async def test_nonstream_overloaded_payload_becomes_retryable_error(self):
        ...
        async with httpx.AsyncClient() as client:
            with (
                patch("optiproxai.proxy._http", client),
    ...
    async def test_streaming_multi_chunk_usage_logs_once(self):
        ...
        async with httpx.AsyncClient() as client:
            with (
                patch("optiproxai.proxy._http", client),
                patch.object(client, "send", side_effect=fake_send),
                patch("optiproxai.proxy.log_execution_event") as mock_log,
            ):
                result = await _proxy_upstream(
                    "https://primary.example/v1",
                    "primary-key",
