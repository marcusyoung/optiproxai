...
def compaction_db(tmp_path: Path, monkeypatch):
    ...
    import optiproxai.compaction_store as store
...
class TestResolveModel:
    ...
    def test_returns_routing_decision_with_correct_model(self):
        from optiproxai.router import RoutingDecision
    ...
    def test_does_not_call_routing_logger(self):
        ...
        router = self._make_router()
        with patch("optiproxai.logger.RoutingLogger.log_decision") as mock_log:
            router.resolve_model(profile="auto", tier="SIMPLE")
...
class TestSessionResolution:
    def test_explicit_header_preferred(self):
        from optiproxai.compaction_store import resolve_session_id
...
class TestSnapshotPersistence:
    def test_save_and_get(self):
        from optiproxai.compaction_store import get_snapshot, save_snapshot
    ...
    def test_duplicate_save_is_idempotent(self):
        from optiproxai.compaction_store import save_snapshot
    ...
    def test_hash_differs_for_different_messages(self):
        from optiproxai.compaction_store import snapshot_hash
...
class TestSummaryLifecycle:
    ...
    def test_enqueue_rejects_covered_count_outside_snapshot_bounds(
        ...
        from optiproxai.compaction_store import enqueue_summary, save_snapshot
    ...
    def test_enqueue_rejects_unknown_snapshot(self):
        from optiproxai.compaction_store import enqueue_summary
...
class TestCompactMessages:
    ...
    def test_basic_compaction(self):
        from optiproxai.compaction import try_sync_compaction
    ...
    def test_token_savings_positive(self):
        from optiproxai.compaction import _estimate_tokens, try_sync_compaction
...
class TestComputeSummaryMaxTokens:
    ...
    def test_default_ratio_normal_range(self):
        from optiproxai.compaction import _compute_summary_max_tokens
    ...
    def test_summary_max_tokens_used_in_generate_summary(self):
        ...
        from optiproxai.compaction import generate_summary
...
class TestEstimateTokens:
    ...
    def test_unknown_model_falls_back_to_cl100k_base(self):
        from optiproxai.compaction import _estimate_tokens, _encoder_cache
...
"""
    ...
    assert (
        "X-Optiproxai-Compaction" not in resp.headers
        or resp.headers.get("X-Optiproxai-Compaction") == "off"
    )
...
def test_compaction_headers_present_when_enabled(
    ...
    """Compaction enabled → X-Optiproxai-Compaction header present."""
    from fastapi.testclient import TestClient
    from optiproxai.proxy import app, configure
    ...
    assert "X-Optiproxai-Compaction" in resp.headers
...
class TestHierarchicalCompactMessages:
    ...
    def test_hierarchical_first_pass_no_prior(self):
        ...
        from optiproxai.compaction import _compact_messages_incremental
...
class TestHierarchicalMergeSummaries:
    ...
    def test_hierarchical_merge_via_concatenation_under_threshold(self):
        ...
        import asyncio
        from optiproxai.compaction import _merge_summaries
...
class TestHierarchicalSnapshotMismatchFallback:
    ...
    def test_hierarchical_snapshot_mismatch_covered_count_boundary(self):
        ...
        import json
        from optiproxai.compaction_store import get_snapshot
...
class TestSessionUpsert:
    ...
    def test_upsert_and_get(self):
        from optiproxai.compaction_store import get_session, upsert_session
    ...
    def test_get_missing_session_returns_none(self):
        from optiproxai.compaction_store import get_session
...
class TestWorkerSingleton:
    ...
    def test_default_worker_is_none(self):
        from optiproxai.compaction import get_worker, set_worker
...
class TestBackgroundCompactionWorker:
    ...
    def test_worker_shutdown_no_tasks(self):
        ...
        from optiproxai.compaction import BackgroundCompactionWorker
    ...
    def test_worker_handles_failure_gracefully(self):
        ...
        import sqlite3
        from optiproxai.compaction_store import _db_path
...
class TestCompactMessagesEdgeCases:
    ...
    def test_empty_message_list(self):
        from optiproxai.compaction import _compact_messages
...
class TestCompactionHeadersNoSession:
    """_compaction_headers omits X-Optiproxai-Compaction-Session when session_id is None."""

    def test_no_session_omits_session_header(self):
        from optiproxai.compaction import CompactionResult
        from optiproxai.proxy import _compaction_headers

        result = CompactionResult(mode="inline", session_id=None, session_mode="none")
        hdrs = _compaction_headers(result)
        assert "X-Optiproxai-Compaction" in hdrs
        assert "X-Optiproxai-Compaction-Session" not in hdrs
    ...
    def test_explicit_session_includes_session_header(self):
        ...
        hdrs = _compaction_headers(result)
        assert hdrs.get("X-Optiproxai-Compaction-Session") == "explicit"
...
class TestNoSessionInlinePath:
    ...
    def test_no_session_inline_mode_fires(self):
        ...
        cfg = OptiproxaiConfig(
            ...
            smart_proxy=SmartProxyConfig(
                context_compaction=ContextCompactionConfig(
                    ...
                    session=SessionConfig(header_name="X-Optiproxai-Session-Id"),
                )
            ),
        )
        ...
        try:
            ...
            async def run():
                with patch(
                    "optiproxai.proxy.generate_summary",
