"""API Key management for optiproxai.
...
from optiproxai.dirs import data_dir
...
def generate_key(name: str) -> str:
    ...
    raw = f"optiproxai-{secrets.token_urlsafe(32)}"
...
def validate_key(raw: str) -> bool:
    ...
    if not raw.startswith("optiproxai-") or len(raw) > 256:
        return False
    h = _hash_key(raw)
    return any(
