"""OptiProxAI dashboard — routing analytics and metrics."""
...
def ingest_stderr_proxy_logs() -> int:
    ...
    with sqlite3.connect(_DASHBOARD_DB_PATH) as conn:
        with open(stderr_log, "r", encoding="utf-8", errors="replace") as f:
            for raw_line in f:
                line = raw_line.strip()
                if "[INFO] optiproxai.proxy: ROUTE " in line:
                    ts, payload = line.split(" [INFO] optiproxai.proxy: ROUTE ", 1)
                ...
                if "[INFO] optiproxai.proxy: USAGE " not in line:
                    continue

                ts, payload = line.split(" [INFO] optiproxai.proxy: USAGE ", 1)
                data = _parse_kv_tokens(payload)
...
def render_dashboard_html(stats: dict[str, Any]) -> str:
    ...
    parts = [
        ...
        f'        <div class="flex items-center gap-3">{logo_svg}<span class="text-lg font-semibold tracking-tight">OptiProxAI</span><span class="hidden sm:inline-block text-sm text-muted-foreground font-normal">Dashboard</span></div>',
        ...
        "        <span>OptiProxAI Router Dashboard</span>",
        '        <a class="hover:text-foreground transition-colors" href="https://github.com/marcusyoung/optiproxai" target="_blank" rel="noopener noreferrer">github.com/marcusyoung/optiproxai</a>',
        "    </div>",
        "</footer>",
        # Scripts
        "    <script>",
        ...
        "            }",
        "            const stored = localStorage.getItem('optiproxai-theme');",
        "            const preferred = stored || (window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light');",
        "            applyTheme(preferred);",
        "            if (toggle) toggle.addEventListener('click', function() {",
        "                const next = html.classList.contains('dark') ? 'light' : 'dark';",
        "                localStorage.setItem('optiproxai-theme', next);",
        "                applyTheme(next);",
