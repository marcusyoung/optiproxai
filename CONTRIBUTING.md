...
optiproxai is a Python 3.13+ project.
...
```toml
optiproxai = "optiproxai.cli:main"
```
...
```bash
uv run optiproxai route "hello world" --config config.yaml
```

Start the proxy:

```bash
uv run optiproxai serve --config config.yaml
```
...
OpenAI-compatible client
  -> FastAPI proxy
  -> API key middleware, if optiproxai keys exist
...
- `OptiProxAI doctor` must not print secrets.
...
optiproxai has two kinds of API keys:

1. Upstream provider API keys
   - Used by OptiProxAI to call providers such as OpenRouter or OpenAI.
...
2. OptiProxAI proxy API keys
   ...
   - Managed through `optiproxai keys`.
   - Optional. If no optiproxai keys exist, proxy auth is disabled for backward compatibility.

Useful commands:

```bash
uv run optiproxai keys add local-dev
uv run optiproxai keys list
uv run optiproxai keys remove local-dev
```
...
When changing `src/optiproxai/proxy.py`, check whether the change affects:
...
`POST /admin/reload-config` is admin-only and uses `OPTIPROXAI_ADMIN_TOKEN`.
...
- Admin auth is separate from regular OptiProxAI proxy API keys.
...
```text
$XDG_STATE_HOME/optiproxai/log/routing-YYYY-MM-DD.jsonl
```

Common local default:

```text
~/.local/state/optiproxai/log/routing-YYYY-MM-DD.jsonl
```

Inspect logs:

```bash
tail -f ~/.local/state/optiproxai/log/routing-$(date +%F).jsonl
```
...
- update `src/optiproxai/config.py`
